...
const EPOCH_THRESHOLD: comptime_int = 1e4 * (1 / ALPHA);
...
fn NeuralNetwork(comptime T: type) type {
    return struct {
        ...
        fn write_results_to_file(this: *This, file: std.fs.File) !void {
            ...
            for (this.test_cases, 0..) |case, case_num| {
                ...
                for (0..NUM_INPUTS) |i| {
                    if (i == NUM_INPUTS - 1)
                        try writer.print("{d:.0}] ", .{case.inputs[i]})
                    else
                        try writer.print("{d:.0}, ", .{case.inputs[i]});
                }
                ...
                for (0..NUM_OUTPUTS) |i|
                    if (i == NUM_OUTPUTS - 1)
                        try writer.print("{d:.0}] ", .{case.outputs[i]})
                    else
                        try writer.print("{d:.0}, ", .{case.outputs[i]});
                ...
                for (0..NUM_OUTPUTS) |i|
                    if (i == NUM_OUTPUTS - 1)
                        try writer.print("{d:.4}] ", .{this.test_results[case_num][i]})
                    else
                        try writer.print("{d:.4}, ", .{this.test_results[case_num][i]});
                ...
                for (0..NUM_OUTPUTS) |i|
                    if (i == NUM_OUTPUTS - 1)
                        try writer.print("{d:.0}]\n", .{this.test_results[case_num][i]})
                    else
                        try writer.print("{d:.0}, ", .{this.test_results[case_num][i]});
            }
            try writer.print("\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n", .{});
            try file.seekFromEnd(0);
        }
        ...
        fn print_results(this: *This) void {
            ...
            for (this.test_cases, 0..) |case, case_num| {
                ...
                for (0..NUM_INPUTS) |i| {
                    if (i == NUM_INPUTS - 1)
                        std.debug.print("{d:.0}] ", .{case.inputs[i]})
                    else
                        std.debug.print("{d:.0}, ", .{case.inputs[i]});
                }
                ...
                for (0..NUM_OUTPUTS) |i|
                    if (i == NUM_OUTPUTS - 1)
                        std.debug.print("{d:.0}] ", .{case.outputs[i]})
                    else
                        std.debug.print("{d:.0}, ", .{case.outputs[i]});
                ...
                for (0..NUM_OUTPUTS) |i|
                    if (i == NUM_OUTPUTS - 1)
                        std.debug.print("{d:.4}] ", .{this.test_results[case_num][i]})
                    else
                        std.debug.print("{d:.4}, ", .{this.test_results[case_num][i]});
                ...
                for (0..NUM_OUTPUTS) |i|
                    if (i == NUM_OUTPUTS - 1)
                        ...
                        std.debug.print("{d:.0}, ", .{this.test_results[case_num][i]});
            }
            std.debug.print("\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n", .{});
        }
    };
